...
const App: React.FC = () => {
  ...
  const [location, setLocation] = useState<LocationInfo>({ latitude: 37.5326, longitude: 126.9900, address: '용산구' });
                  ...
                  <div className="pt-2">
                      ...
                      {isLoadingMissions ? (
                          <div className="bg-white rounded-3xl p-8 border border-slate-100 text-center">
                              ...
                              <p className="text-sm text-slate-500">AI가 주변 환경을 분석하고 있어요...</p>
                          </div>
                      ) : !hasSynced ? (
                          <div className="bg-white rounded-3xl p-8 border border-slate-100 text-center flex flex-col items-center">
                              ...
                              <button onClick={() => refreshData(location.latitude, location.longitude)} className="mt-2 text-xs text-emerald-600 underline">연동 시작하기</button>
                          </div>
                      ) : missions.length === 0 ? (
                          <div className="bg-white rounded-3xl p-8 border border-slate-100 text-center flex flex-col items-center">
      ...
      {showMapModal && (
        <div className="absolute inset-0 bg-white z-50 flex flex-col animate-in slide-in-from-bottom-full duration-300">
           ...
           <div className="p-4 bg-slate-50 border-b border-slate-100 flex gap-2">
              ...
              <button onClick={handleMapSearch} className="bg-emerald-600 text-white px-4 rounded-xl font-bold text-sm whitespace-nowrap">검색</button>
           </div>
